...
public final class PasswordGenerator {
    ...
    public static class PasswordGeneratorBuilder {
        ...
        private boolean usePunctuation;
/**
        public PasswordGeneratorBuilder(boolean x , boolean y,boolean z,boolean w) {
            x = false;
            y = false;
            z = false;
            w = false;
 ...
 */
    ...
    public String generate(int length) {
        ...
        for (int i = 0; i < length; i++) {
            // System.out.println(charCategories.size()); --> check size
    ...
    public static void main(String args[]){
        ...
        System.out.println("Use digits (y/n)");
        String digit = inputObj.nextLine();
        if (digit.equals("y")){
            d = true;

        } else {d = false;}
        System.out.println("Use lower cases (y/n)");
        String lower = inputObj.nextLine();
        if (lower.equals("y")){
            l = true;
        } else {l = false;}
        System.out.println("Use upper cases (y/n)");
        String upper = inputObj.nextLine();
        if (upper.equals("y")){
            u = true;
        } else {u = false;}
        System.out.println("Use punctuation (y/n)");
        String punctuation = inputObj.nextLine();
        if (punctuation.equals("y")){
            p = true;
        } else {p = false;}
       ...
       // System.out.println("lowercase" + l + "uppercase" + u + ".");
        ...
        PasswordGenerator passwordGenerator = new PasswordGeneratorBuilder(l, u, d, p)
                .useLower(l)
                .useUpper(u)
                .useDigits(d)
                .usePunctuation(p)
                .build();
        System.out.println("How many characters would you like to put in the password");
        int leng = inputObj.nextInt();
        String password = passwordGenerator.generate(leng);
